...
- **Deployment**: both Vercel (frontend) and Railway (backend) are linked to GitHub and auto-deploy on push to `main`. **Do not run `vercel --prod` manually** — push to `main` and the deploy lands within ~60s. Same for Railway: push triggers `alembic upgrade head` + uvicorn restart on the server.
