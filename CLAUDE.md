...
- **Visual register — La Grille · Soft warmth** locked by [ADR-0004](docs/adr/0004-modern-sober-refresh.md). Geist + Geist Mono on `#FAFAF7` off-white, refined terracotta `#A8523C` reserved for state, numbered indices as the composition keystone, hairline cards (no shadow except the deck card), table-à-manger logomark. Tokens live in `frontend/app/globals.css`. Validated per-area decisions in `.claude/skills/sketch-findings-al-dente/SKILL.md` (auto-loads on UI work). Visual reference HTML at `.planning/sketches/002-refresh-direction-explorations/index.html`. **`docs/design-system.html` is historical** — it documents the retired Sober Kitchen register (Cormorant + Caveat + paper-grain + patine + warm-brown shadows); do NOT mirror its tokens into new code.
...
5. **Raw inputs kept forever.** Per [ADR-0001](docs/adr/0001-recipe-conversation-thread.md), the `recipe_turns` table stores original transcripts / URLs / photo paths immutably — the first user turn (position 0) of each recipe preserves the capture payload verbatim so prompts can be re-run with a better model later. The legacy `recipes.source_capture` JSONB column was dropped in Alembic migration `0009` (Phase 25 THREAD-01); don't reintroduce per-recipe blob fields for raw inputs — thread turns are the durable store.
...
- Backend: snake_case (e.g., `promotion_error`, `manually_edited_fields`, `extracted_html_path`, `photo_paths`)
