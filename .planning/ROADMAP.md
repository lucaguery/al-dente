...
**Plans**: 3 plans
- [x] 17-01-PLAN.md — backend TZ-01 fix at start_cooking + get_active_cooking_log via household.timezone/zoneinfo + new GET /cooking-logs list + GET /cooking-logs/{log_id} detail + 10 pytest regressions (FIX-01, HIST-01)
- [x] 17-02-PLAN.md — frontend fetchCookingLogs / fetchCookingLog helpers + new /cooking-logs/[id] detail page (paper-grain Card, Fraunces italic absolute-date header, signed photo, rating chip, recipe back-link) (HIST-02)
- [x] 17-03-PLAN.md — list page rewires to fetchCookingLogs + tap-to-detail navigation + un-test.fixme both cooking-log-history.spec.ts AND cooking-log-create-finalize.spec.ts + detail-navigation e2e assertion (HIST-01, HIST-02, FIX-01)
**UI hint**: yes
**Key risks / threat-model flags**:
