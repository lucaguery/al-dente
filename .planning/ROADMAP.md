...
Run `/gsd-new-milestone` to scaffold the next milestone. The validated "La Grille Completion" scope (8 unimplemented sketch screens from `.claude/skills/sketch-findings-al-dente/sources/002-refresh-direction-explorations/index.html` + cooking-logs token drift fix) is the leading candidate — recommended split into 3 risk-graduated phases (pure-frontend restyles / cross-cutting features w/ first backend touch / new-data-model + active cooking session).
...
*Last updated: 2026-05-21 — v0.8 Backend Coverage Until Done shipped (3 phases, 9 plans, 33/33 requirements). Repo coverage 35.9% → 85.08%; gh#28 closed. Next: `/gsd-new-milestone` to scaffold v0.9 (recommended scope: La Grille Completion — 8 sketch screens + cooking-logs token drift fix, 3 phases).*
