...
**Plans:** 5 plans
Plans:
- [ ] 27-01-PLAN.md — Backend cleanup: POST /recipes empty body + POST /recipes/{id}/promote (D-13b) + delete 4 legacy capture endpoints + drop legacy frontend helpers (CAPTURE-03)
- [ ] 27-02-PLAN.md — Build shared RecipeThread component directory (orchestrator + Bubble + SystemBubble + Composer + VoiceSheet + UrlSheet + PhotoMenu + types) + recipes.thread.* i18n namespace (CAPTURE-01, CAPTURE-02, CAPTURE-04)
