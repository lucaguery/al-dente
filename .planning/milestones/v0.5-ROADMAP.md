# Roadmap Archive: v0.5 Mixed Sweep
...
**Success Criteria** (what must be TRUE):
  1. Dragging a shortlist card in either direction shows a full-card color tint (emerald-tinted for yes, destructive-tinted for no) with no OUI/NON text overlay visible anywhere on the card — *shipped as a `ring-2 ring-inset` stroke fade-in per Phase 23 D-01 / SE-1 (Tailwind plain `ring-*` is clipped by `overflow-hidden`); semantically equivalent feedback channel*
...
- **#10 silent overwrite** — LLM title rewrite replaces user input in place via `BackgroundTask`; title remains editable on detail page. No new confirmation UI to ship.
- **#10 failure mode** — on `rewrite_title()` failure, keep user title and set `promotion_error` for retry (mirrors v0.4 Phase 16 failed-state handling; invariant #5 preserved). Rewrite-failure path lands as `status='structured'` (not `'failed'`) — recipe is usable without catchy title (D-26 asymmetry).
- **#17 icon direction** — filled Heart / outline Heart; emerald (`--color-valide-foreground`) for filled, neutral for outline. Single-glyph language; softer than thumbs.
- **Invariant #1 shift** — quick/full-form move from sync `structured`-on-return to async `BackgroundTask` rewrite when Phase 24 ships #10. `CLAUDE.md` invariant #1 wording updated in the same atomic commit (`5e6a2ff`) per D-30.
- **DECK-01 design deviation (Phase 23 D-01)** — "full-card color tint" rewritten to "drag-distance-driven `ring-2 ring-inset` fade-in" mid-phase after Tailwind `overflow-hidden` clipped plain `ring-*` (RESEARCH SE-1). REQUIREMENTS.md updated in same commit.
- **RID-05 SVG sanitizer (Phase 24 D-33)** — stdlib `xml.etree.ElementTree` (Python 3.12 ET safe against XXE) chosen over lxml (absent) and defusedxml (redundant). Reject-and-fallback only — no strip-and-keep mode. 28 unit tests pin the allowlist.

**Issues Resolved:**

- gh#10 — LLM "catchy" French title rewrite across all capture surfaces; quick + full-form move to async `BackgroundTask` shape
- gh#11 — `BrandIcon` component extracted from `app/icon.tsx`; mounted on welcome + 3 empty states
- gh#12 — Per-recipe SVG illustration pipeline with strict allowlist sanitizer (28 unit tests) + `dangerouslySetInnerHTML` trust boundary + `BrandIcon` fallback
- gh#13 — Geist Mono dependency dropped entirely; `tabular-nums` carries the code signal
- gh#14 — OUI/NON drag overlays replaced by `ring-2 ring-inset` color-tinted fade-in
- gh#15 — Build-time `VersionFooter` rendering `v{version} · {sha} · {env}` at the bottom of /settings
- gh#16 — Tap-to-detail on shortlist cards (`useRouter` + `panRef` discrimination)
- gh#17 — Filled/outline Heart thumb-buttons replacing thumbs-up/down
- gh#18 — Swipe threshold + spring retune (`SWIPE_THRESHOLD_PX` 100→140, `SWIPE_VELOCITY_PX_S` 500→750, `SWIPE_FLYOFF_DURATION_S` 0.2→0.28)
- gh#21 — `useEnumLabels()` wired into ShortlistCard + recipe detail page for French cuisine/mood/protein tags
- gh#22 (Part A) — Three new optional recipe fields (`cook_time_minutes`, `difficulty` with CHECK, `description`) + `Difficulty` enum locked on both sides
- gh#22 (Part B) — `CompletenessCard` with 11-field equal-weight scoring + `?focus=` ref-based edit-page navigation

**Issues Deferred:**

- gh#20 (unified capture surface) → v0.6 (needs its own `/gsd-explore` UX cycle; currently `needs-info`)
- gh#12 "regenerate-on-demand UI", detail/shortlist illustration placements, SVG animation → future tickets
- gh#22 inbox-row completeness ring, weighted scoring, backfill migration → v2 / pending usage data
- #10 URL-surface title rewrite — URL extraction remains `# TODO(productize)` (URL-01 backlog)

**Technical Debt Incurred:**

- **WR-01** (idempotent double `db.close()` in `retry_promotion`): acknowledged at couple-scale; benign in practice (SQLAlchemy `close()` is idempotent), can revisit if it ever leaks.
- **WR-02** (prod-synthetic seed misses Phase 24 fields on 18 of 21 recipes): intentional low-completeness nudge per D-16; only 3 recipes carry the new fields. Future seed refresh can backfill.
- **WR-03** (BackgroundTask reads `recipe.title` not `source_capture.payload.title`): edit-race trace exists at couple-scale — accepted per D-29 (silent overwrite always wins). Documented.
- **DECK-01 visual gap from REQ wording** — REQUIREMENTS.md text updated in-commit to reflect the ring-stroke implementation; future surfaces using the same `overflow-hidden` ancestor must reach for `ring-inset` directly.
- **15 v0.4 HUMAN-UAT items** remain orthogonal to v0.5 — tracked via `/gsd-audit-uat`.
