# Requirements Archive: v0.7 Sober Kitchen + Polish
...
- [x] **SOBER-07** — Marginalia register applied — Caveat handwriting carries the human-voice annotations (manual-edit pin labels, system asides, register cues) consistently across the locked screens. Phase 28's `PinLabel.tsx` is the precedent.
- [x] **SOBER-08** — Brand-mark loader replaces ad-hoc spinners on slow-path surfaces. Loading states route through the locked brand-mark animation per the doc.
