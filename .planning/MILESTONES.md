...
## v0.9 La Grille Completion (Shipped: 2026-05-21)

**Phases completed:** 3 phases (40–42), 14 plans, 19/19 active requirements validated (SPLA-02 deferred per Phase 40 D-09 — iOS apple-touch-startup-image matrix)

**Stats:**

- Timeline: 2026-05-21 (1 calendar day)
- Closes the sketch-002 vs implementation gap surfaced 2026-05-21 (8 unimplemented sketch screens + cooking-logs token drift)
- Final: 19 active requirements shipped across PROF / ONBO / LIB / SPLA / DRIFT / THRD / PICK / UNDO / STEP / ACTV
- Deferred to v0.10+: SPLA-02 (iOS PWA boot image matrix), bidirectional partner-sync UX polish (HomeDecide votes[] filter expansion noted in 41-04 SUMMARY)

**Key accomplishments:**

- **Phase 40 — Pure-Frontend Restyles (5 plans):** Profil page (`/settings`) rewritten to literal-sketch composition — partner block + stats block + 5 numbered hairline rows (`01 Notifications` / `02 Foyer` / `03 Membre` / `04 Exporter` / `05 Déconnexion`), no Card components anywhere. NEW `GET /households/{id}/stats` backend endpoint (the one backend touch of an otherwise frontend-only phase) returns all-time household counts (recipes / cooking_logs / votes); joins through Member for vote count per the v0.8 Phase 38 finding that `Vote.household_id` doesn't exist. Onboarding welcome rewritten to wordmark-centric composition (Al Dente. + dot + italic-emphasis tagline + primary filled-dark + ghost hairline button pair + footer marketing line); no Cards. Library `LibraryViewSwitch` gains a 3rd `minimal` mode with new `RecipeRowMinimal` (no photo column); selection persists in localStorage. `app/loading.tsx` ships the La Grille splash composition (table-à-manger logomark + wordmark + tagline + 3-dot loader + version footer). `cooking-logs/[id]/page.tsx` token drift fixed — Fraunces italic + `bg-surface-rose-100` + `bg-[var(--color-valide-tint)]` direct refs all replaced with La Grille tokens (loved rating chip now uses `bg-[var(--color-valide-tint)] text-primary border border-primary` per D-10 — mirrors Validated state language). SPLA-02 iOS apple-touch-startup-image matrix deferred per D-09 (`loading.tsx` only ships for v0.9).
- **Phase 41 — Navigation Surgery + First Backend Touch (4 plans):** Recipe thread becomes a dedicated route — new `app/recipes/[id]/thread/page.tsx` mounts `<RecipeThread mode="detail" />`; inline thread rendering ripped out of the structured view at `[id]/page.tsx`; structured view gains a `N tours` Geist Mono pin in `det-top` that routes to `/thread`. Nouvelle Recette becomes a route-level chooser — `/recipes/new` is a 5-option picker (`01 Note rapide` → name-only modal that POSTs blank-name to `/recipes` + redirects to `/recipes/{id}`, the other 4 enter `/recipes/new/[surface]` with the thread composer pre-seeded for that capture mode); the in-thread D-09 + D-11 unification (no tabs INSIDE the thread) is preserved. `ShortlistThumbButtons` grows from 2 to 3 buttons (X / RotateCcw / Heart) with the middle button disabled when there's no vote to undo OR when the veto window has closed (preemptive tooltip from local state — backend 409 is defense-in-depth). NEW `DELETE /votes/{vote_id}` endpoint adheres to the v0.8 Phase 38 5-test contract (happy / 401 / 404-cross-household / 404-not-found / 409-veto-window-closed) plus invariant #2 regression test (`test_no_state_column_on_votes_or_recipes` — DELETE removes a row; no state column was added). POST `/votes` extended to return `vote_id` so the client can hold it for later DELETE. NEW `vote.deleted` broadcast event extends invariant #4; frontend `RealtimeProvider` gains the listener (orchestrator-closed inline after the executor correctly refused to expand scope outside Plan 41-04's `files_modified` for `RealtimeProvider.tsx`).
- **Phase 42 — Structured Steps + Active Cooking Session (5 plans):** Alembic migration 0013 ALTERs `recipes.steps` from nullable JSONB to `NOT NULL DEFAULT '[]'::jsonb` + UPDATE backfill on existing rows (the column already existed, RESEARCH §R-01 caught this — migration is ALTER not ADD). Pydantic `StepEntry` schema (`{text, ingredient_refs: string[]}`) added to `backend/app/schemas/recipe.py`; mirrored as TS `StepEntry` in `frontend/lib/recipes.ts` (locked-vocabularies discipline — drift = bug). Gemini prompt-schema extended with structured `steps: list[StepEntry]` — `_apply_extracted` serialization fixed via `[s.model_dump(mode="json") for s in (extracted.steps or [])]` (RESEARCH §R-03 caught the type-incompatibility with the existing `string[] | None` write). NEW `POST /recipes/{id}/extract-steps` endpoint schedules a `BackgroundTask` (`extract_and_persist_steps`) mirroring the invariant #1 server-side promotion pattern; broadcasts `recipe.updated` after committing (invariant #4); turn-0 immutability regression test (invariant #5) green. NEW `app/cooking-logs/[id]/active/page.tsx` route (274 LOC) renders the sketch §Cuisine session active composition — det-top with `cooked_at`-based start-time crumb (RESEARCH §R-02 corrected `started_at` → `cooked_at`), terracotta progress segments (prior 15% tint / current solid / future hollow), step text + ingredient-ref line via name-match graceful fallback, prev/next navigator with `Terminé · marquer cuisinée` CTA on the last step routing to existing `/cooking-logs/{id}/finalize` (D-17). Lazy backfill on first /active visit handles empty + legacy `string[]` step rows; `BrandLoader` shows "Préparation des étapes…" until `recipe.updated` lands.

**Process notes:**

- Background-agent stall rate: 4 of 7 dispatches (57%) — plan-41 (1 of 4 plans missing), execute-41 (verification step missing), execute-42 first try (1 of 5 plans done), execute-42 retry (4 of 5 plans done, /active page uncommitted). In all four cases the orchestrator inline-closure pattern recovered cleanly; the watchdog kept partial work safe; the executor's atomic-commit discipline made recovery trivial.
- Two CONTEXT.md discuss-time decisions corrected by planner research (Phase 42 R-01 column-already-exists, R-02 field-is-cooked-at) — this is exactly what gsd-phase-researcher is meant to do: ground user intent against the actual codebase before plans are committed.
- Plan 41-04 had a `files_modified` omission (`RealtimeProvider.tsx` for `vote.deleted` listener); orchestrator closed the gap inline. Plan 42-04 silently expanded scope (3 type-alignment files outside `files_modified`); the orchestrator accepted because the changes were necessary type-contract follow-through, not scope creep.
- Schema refinement during discuss: PROJECT.md locked STEP-01 as "nullable column"; discuss-time D-01 refined to `NOT NULL DEFAULT '[]'::jsonb`. Backfill condition is `jsonb_array_length(steps) = 0`.

**Known deferred items at close:** SPLA-02 (iOS apple-touch-startup-image matrix per D-09); HomeDecide votes[] filter expansion for `deleted: true` flow (41-04 SUMMARY); future v0.10+ items captured in each phase's CONTEXT.md Deferred Ideas (cooking step images, wake-lock during /active, voice-controlled step navigation, multi-step undo history, step timers per step, per-member stats splits, last-30-day stats windows).
