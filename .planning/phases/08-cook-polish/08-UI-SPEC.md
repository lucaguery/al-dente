...
| RatingPicker helper (`On la refait sans hésiter` etc.) | `text-sm text-foreground-muted leading-5` (IBM Plex Sans 400) | **Phase 8 size-fold:** the previous `text-xs leading-4` (12/16) is folded into `text-sm leading-5` (14/20) so the Phase 8 type scale resolves to exactly 4 sizes (`text-display` / `text-title` / `text-base` / `text-sm`). The helper line gains 2px of body and 4px of leading; legibility on iPhone PWA improves with no layout regressions (`h-20` rating card has ample vertical room). Executor must update `frontend/components/RatingPicker.tsx:83` to match: `<span className="text-sm text-foreground-muted leading-5">`. |

**Type scale used in Phase 8:** **4 distinct sizes** — `text-display` (32–44), `text-title` (24), `text-base` (16), `text-sm` (14). **No `text-xs` (12px) on Phase 8 surfaces.** The previous Phase 4/7 RatingPicker helper line that consumed `text-xs leading-4` is folded into `text-sm leading-5` as part of this phase's polish (see RatingPicker row above and §Component Inventory `RatingPicker.tsx` change). **No new sizes added by Phase 8** — the type scale is bounded by the Phase 5 4-class system (`text-display`, `text-title`, `text-body`, `text-caption`) plus the standard Tailwind `text-sm` consumed for chrome / helpers / inline-meta, totalling 4 sizes.

**Weights — inherited Phase 5 system invariant (NOT a new Phase 8 declaration).** Phase 5 §Typography established the system-level weight contract verbatim: *"400 (body) + 500 (display/title) + 600 (locked usages — submit CTAs, locked-rating labels, vote-chip pills). This is 3 weights total but each has a single reserved purpose; the discipline holds."* Phase 6 and Phase 7 inherited the same 3-weight roster under that exception. **Phase 8 introduces zero new weight tokens and zero new role assignments.** The same three reserved purposes apply on cook surfaces: 400 carries running text (ingredient lines, step bodies, RatingPicker helpers, footers, captions); 500 carries display + title + step-number Fraunces-italic prefix + chip-pill labels + Badge default; 600 carries CTA labels + section headings + list-row titles. **No fourth weight is introduced. The 3 weights pre-exist this phase and are restated here only for executor reference.**
...
Pill shape contract: `inline-flex items-center rounded-full px-2 py-1 text-sm font-medium h-8` — **all four spacing values (`px-2` = 8px, `py-1` = 4px, `h-8` = 32px) are strict 4-multiples** matching Phase 5 §Spacing. Same shape register as Phase 7 vote chips (consistency across cooking surfaces). **No new i18n keys** — the rating labels reuse existing `cooking_log.rating.loved` / `cooking_log.rating.liked` / `cooking_log.rating.disliked` keys.
...
- Passer: `Button variant="ghost" size="sm" className="h-12 px-3"` → `Button variant="ghost" className="h-12"` (remove redundant `size="sm"` and `px-3`; height is now solely from explicit className; the shadcn Button primitive's size-default horizontal padding carries through — no Phase 8 sub-4px spacing token is declared in this surface contract).
...
- [ ] Dimension 4 Typography: PASS (**4 sizes** on Phase 8 surfaces — `text-display` / `text-title` / `text-base` / `text-sm`; no `text-xs` consumers in Phase 8 — RatingPicker helper folded into `text-sm leading-5`; **3-weight roster INHERITED from Phase 5 system invariant** verbatim — *"This is 3 weights total but each has a single reserved purpose; the discipline holds."* — Phase 8 adds zero weight tokens; per-element role assignment provided; recipe-detail uses Fraunces display + title + step-number italic prefix; library/history list rows stay sans for chrome register)
