...
**Weights — inherited Phase 5 system invariant (NOT a new Phase 9 declaration).** Phase 5 §Typography established the system-level weight contract verbatim: *"400 (body) + 500 (display/title) + 600 (locked usages — submit CTAs, locked-rating labels, vote-chip pills). This is 3 weights total but each has a single reserved purpose; the discipline holds."* Phases 6/7/8 inherited the same 3-weight roster under that exception. **Phase 9 introduces zero new weight tokens and zero new role assignments.** The same three reserved purposes apply on Phase 9 surfaces: 400 carries running text (taglines, helpers, errors, field values, body); 500 carries display-italic + title + nav labels + badge content + Card-title Fraunces-italic + chipClass register; 600 carries CTA labels + section headings + sticky-header chrome. **No fourth weight is introduced. The 3 weights pre-exist this phase and are restated here only for executor reference.**
...
- [ ] Dimension 4 Typography: PASS (4 sizes — `text-display`, `text-title`, `text-base`, `text-xs`; 3 weights inherited from Phase 5 system invariant — 400 / 500 / 600 with locked usages; Fraunces italic invite-code display register specified verbatim for share-code + Settings; BottomNav label normalized to `text-xs` — 4-multiple line-height; weights framed as Phase 5 inheritance not Phase 9 declaration)
